...
// ValueFiles represents a list of values files paths to load and merge
type ValueFiles []string

// ValueFiles.String() prints the list of values files paths
func (v *ValueFiles) String() string {
	return fmt.Sprint(*v)
}

// ValueFiles.Type() should have a comment
func (v *ValueFiles) Type() string {
	return "ValueFiles"
}

// ValueFiles.Set() should have a comment
...
func readFile(settings environment.EnvSettings, filePath, CertFile, KeyFile, CAFile string) ([]byte, error) {
...
// MergeValuesFiles merges values from files specified via -f/--values and
// directly via --set or --set-string or --set-file, marshaling them to YAML
func MergeValuesFiles(settings environment.EnvSettings, valueFiles ValueFiles, values []string, stringValues []string, fileValues []string, CertFile, KeyFile, CAFile string) ([]byte, error) {
	base := make(Values)

	// User specified a values files via -f/--values
	for _, filePath := range valueFiles {
		currentMap := make(Values)
		...
		if strings.TrimSpace(filePath) == "-" {
			...
			bytes, err = readFile(settings, filePath, CertFile, KeyFile, CAFile)
		}
	...
	for _, value := range fileValues {
		reader := func(rs []rune) (interface{}, error) {
			bytes, err := readFile(settings, string(rs), CertFile, KeyFile, CAFile)
